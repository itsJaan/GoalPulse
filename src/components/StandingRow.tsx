...
export const StandingRow = ({ entry, isFavorite }: { entry: StandingEntry; isFavorite?: boolean }) => (
  <View style={[styles.row, isFavorite && styles.rowHighlight]}>
    {entry.zone && (
      <View style={[styles.zoneBar, { backgroundColor: ZONE_COLORS[entry.zone] }]} />
    )}

    <Text style={[styles.position, isFavorite && styles.positionHighlight]}>
      {entry.position}
    </Text>
    ...
    <View style={styles.teamBlock}>
      <TeamLogo
        uri={entry.team.logo}
        size={24}
        fallback={entry.team.shortName?.slice(0, 3).toUpperCase()}
      />
      <Text
        style={[styles.teamName, isFavorite && styles.teamNameHighlight]}
        numberOfLines={1}
      >
    ...
    <Text style={[styles.stat, styles.gd]}>
      {entry.goalDiff > 0 ? `+${entry.goalDiff}` : entry.goalDiff}
    </Text>
    <Text style={[styles.stat, styles.points, isFavorite && styles.pointsHighlight]}>
      {entry.points}
    </Text>
